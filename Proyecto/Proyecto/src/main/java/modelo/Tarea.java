...
public class Tarea extends Elemento
{
    ...
    public boolean isEsRepetitiva(){return this.esRepetitiva;}
    ...
    public void completarSubTarea(int index){
        if (index >= 0 && index < subtareas.size()){
            subtareas.get(index).completar();
            actualizarEstado();
        }
    }
        ...
        for (Elemento e : tareas)
        {
            ...
            if (e instanceof Tarea)
            {
                ...
                if (!t.getSubtareas().isEmpty())
                {
                    ...
                    {
                        System.out.println(i + ". " + s.getTitulo() + " - " + (s.isCompletada() ? "✔" : "✘"));
                        i++;
                    }
                }
            }
